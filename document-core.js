...
(function (root) {
  ...
  const definitions = [
    {
      key: 'poliza',
      label: 'PÓLIZA',
      pattern: /(?:N\s*[°ºO.*#]*\s*(?:DE\s*)?|NUMERO\s+DE\s+)POLIZA\s*:?/g,
    },
    ...
    {
      key: 'suma',
      label: 'SUMA ASEGURADA BÁSICA',
      pattern: /SUMA\s+ASEGURADA\s+BASICA\s*(?:\(\s*BAS\s*\))?\s*:?/g,
    },
    ...
    { key: 'trabajo', label: 'NOMBRE DE LA EMPRESA', pattern: /NOMBRE\s+DE\s+LA\s+EMPRESA\s*:?/g },
    {
      key: 'comunidad',
      label: 'LUGAR Y FECHA',
      pattern: /LUGAR\s+Y\s+FECHA(?!\s+DE\s+NACIMIENTO)\s*:?/g,
    },
  ];
  ...
  function searchClients(items, key, query) {
    ...
    return items
      .filter((r) => compare(r[key]).includes(term))
      .sort(
        (a, b) =>
          Number(compare(b[key]) === term) - Number(compare(a[key]) === term) ||
          String(b._saved || '').localeCompare(String(a._saved || '')),
      );
  }
  ...
  function sanitize(key, value) {
    ...
    if (key === 'comunidad')
      return v
        .replace(/(?:,?\s+(?:A\s+)?\d{1,2}(?:\s*DE\s*|[/.-]).*)$/i, '')
        .replace(/,?\s+(?:VER\.?|VERACRUZ)\.?$/i, '')
        .trim()
  ...
  function valuesToForm(values) {
    ...
    definitions.forEach((d) => {
      if (!['paterno', 'materno', 'nombres'].includes(d.key))
        result[d.key] = sanitize(d.key, values[d.key]);
    });
    result.nombre = ['paterno', 'materno', 'nombres']
      .map((k) => sanitize(k, values[k]))
      .filter(Boolean)
      .join(' ');
  ...
  function anchors(text) {
    ...
    for (const def of [...definitions].sort((a, b) => b.label.length - a.label.length)) {
      const pattern = new RegExp(def.pattern.source, 'g');
      let match;
      while ((match = pattern.exec(upper))) {
        const start = match.index,
          end = start + match[0].length;
        if (!hits.some((h) => start < h.end && end > h.start))
          hits.push({ key: def.key, start, end });
      }
    }
    // Stop extraction at other known form labels; never consume the rest of the page.
    const stop =
      /\b(?:PRIMA(?!\s+EX[CE]*DENTE)|MEDIO|VENDIDA|TELEFONO\s+FIJO|DOMICILIO|SEXO|FECHA\s+DE\s+NACIMIENTO|FIRMA|BENEFICIARIOS?|DATOS\s+DEL|LUGAR\s+Y\s+FECHA\s+DE\s+NACIMIENTO|ESTADO\s+CIVIL|EDAD|ESTATURA|PESO|OCUPACION|SUELDO\s+MENSUAL|POLIZA\s+NUEVA|INCREMENTO|INCLUSION)\b/g;
    let m;
    while ((m = stop.exec(upper)))
      if (!hits.some((h) => m.index >= h.start && m.index < h.end))
        hits.push({ key: null, start: m.index, end: m.index + m[0].length });
  ...
  function extractText(text) {
    const values = {},
      hits = anchors(text);
    hits.forEach((hit, i) => {
      if (!hit.key || values[hit.key]) return;
      const fragment = text.slice(hit.end, hits[i + 1]?.start ?? text.length);
      const lines = fragment.split(/\r?\n/).map(clean).filter(Boolean);
      values[hit.key] = sanitize(hit.key, lines[0] || '');
    });
    const marked = [];
    ...
    const markedBox = /[X☑☒✓✔✗✘]/;
    const prefix =
      choices.length && new RegExp(box + '\\s*$').test(upper.slice(0, choices[0].index));
    choices.forEach((choice, index) => {
      const fragment = prefix
        ? upper.slice(
            index ? choices[index - 1].index + choices[index - 1][0].length : 0,
            choice.index,
          )
        : upper.slice(choice.index + choice[0].length, choices[index + 1]?.index ?? upper.length);
      const match = fragment.match(new RegExp(prefix ? box + '\\s*$' : '^\\s*' + box));
      if (match && markedBox.test(match[0]))
        marked.push(choice[0] === 'POLIZA NUEVA' ? 'NUEVA' : choice[0]);
    });
    values.negocio = marked[0] || '';
    return {
      values,
      marked,
      warning: marked.length > 1 ? 'Hay varias casillas marcadas; revisa NEGOCIO.' : '',
    };
  }
  ...
  function fieldRegions(words, width, height) {
    ...
    const ranges = words.map((w) => {
      const r = { ...w, start: offset, end: offset + w.text.length };
      offset = r.end + 1;
      return r;
    });
    return anchors(text)
      .filter((h) => h.key)
      .map((hit) => {
        const label = ranges.filter((w) => w.end > hit.start && w.start < hit.end);
        if (!label.length) return null;
        const x0 = Math.min(...label.map((w) => w.bbox.x0)),
          x1 = Math.max(...label.map((w) => w.bbox.x1)),
          y0 = Math.min(...label.map((w) => w.bbox.y0)),
          y1 = Math.max(...label.map((w) => w.bbox.y1));
        const lineHeight = Math.max(12, y1 - y0);
        const right = ranges.filter(
          (w) => w.bbox.x0 > x1 && Math.abs(w.bbox.y0 - y0) < lineHeight * 0.65,
        );
        const otherLabel = anchors(text)
          .filter((h) => h.key !== hit.key)
          .flatMap((h) => ranges.filter((w) => w.end > h.start && w.start < h.end));
        const nextX = Math.min(
          width,
          ...otherLabel
            .filter((w) => w.bbox.x0 > x1 && Math.abs(w.bbox.y0 - y0) < lineHeight * 2)
            .map((w) => w.bbox.x0 - 5),
        );
        const values = right.filter(
          (w) => w.bbox.x0 < nextX && !otherLabel.some((label) => label.start === w.start),
        );
        let rect = {
          left: Math.max(0, x0 - 3),
          top: Math.max(0, y1 + 2),
          width: Math.max(20, Math.min(nextX - x0, width * 0.38)),
          height: Math.min(height - y1 - 2, lineHeight * 2.8),
        };
        if (values.length) {
          const left = Math.max(0, Math.min(...values.map((w) => w.bbox.x0)) - 4),
            top = Math.max(0, Math.min(...values.map((w) => w.bbox.y0)) - 3);
          rect = {
            left,
            top,
            width: Math.min(width, Math.max(...values.map((w) => w.bbox.x1)) + 4) - left,
            height: Math.min(height, Math.max(...values.map((w) => w.bbox.y1)) + 3) - top,
          };
        }
  ...
  function extractLayout(data, width, height) {
    const result = extractText(data.text || ''),
      words = flattenWords(data),
      regions = fieldRegions(words, width, height);
    ...
    for (const region of regions) {
      const b = region.label,
        h = Math.max(12, b.y1 - b.y0);
      // Names are separate columns; the nearest writing line may be above or below its printed label.
      if (!['paterno', 'materno', 'nombres'].includes(region.key) || b.y0 > height * 0.5) continue;
      if (region.inline && result.values[region.key]) continue;
      const peers = regions.filter(
        (r) => r.key !== region.key && Math.abs(r.label.y0 - b.y0) < h * 1.5,
      );
      const right = Math.min(
        width,
        ...peers.filter((r) => r.label.x0 > b.x0).map((r) => r.label.x0 - 4),
      );
      const candidates = words.filter(
        (w) =>
          !isLabel(w) &&
          w.bbox.x0 >= b.x0 - h * 0.5 &&
          w.bbox.x1 <= right &&
          Math.abs(w.bbox.y0 - b.y0) < h * 3 &&
          Math.abs(w.bbox.y0 - b.y0) > h * 0.5,
      );
      if (!candidates.length) {
        result.values[region.key] = '';
        continue;
      }
      candidates.sort((a, c) => Math.abs(a.bbox.y0 - b.y0) - Math.abs(c.bbox.y0 - b.y0));
      const baseline = candidates[0].bbox.y0;
      const row = candidates
        .filter((w) => Math.abs(w.bbox.y0 - baseline) < h * 0.65)
        .sort((a, c) => a.bbox.x0 - c.bbox.x0);
