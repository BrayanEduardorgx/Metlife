  ...
  .createServer(async (req, res) => {
    try {
      ...
      if (req.url === '/api/records' && req.method === 'POST') {
        ...
        if (!record._id)
          record._id = `record_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
        recordQueue = recordQueue.then(async () => {
          const records = await getRecords();
          if (!records.some((item) => item._id === record._id)) {
            records.push(record);
      ...
      const file = join(
        process.cwd(),
        req.url === '/' ? 'index.html' : decodeURIComponent(req.url).replace(/^\//, ''),
      );
