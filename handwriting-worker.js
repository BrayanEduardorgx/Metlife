...
self.onmessage = async ({ data }) => {
  ...
  try {
    model ||= pipeline('image-to-text', 'Xenova/trocr-small-handwritten', {
      quantized: true,
      progress_callback: (event) => {
        if (event.status === 'progress')
          self.postMessage({ id, progress: Math.round(event.progress || 0) });
      },
    }).catch((error) => {
      model = null;
      throw error;
    });
    const reader = await model;
    ...
    self.postMessage({
      id,
      error:
        'No se pudo cargar o ejecutar el lector de letra a mano. Prueba Lectura rápida o corrige el campo manualmente.',
    });
  }
};
